...
Servo channels in bbrx will output this type of signal.  Since every ESC is different, you can specify the PWM parameters on a per-binding basis using the [`servo_pwm_max`, `servo_pwm_min`, `servo_pwm_mid`](config.md#bindings) parameters.  These parameters are optional, and they'll use defaults specified in [config.h](../bbrx/config.h):
