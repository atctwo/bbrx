...
void leds_set_brightness(uint8_t brightness) {
    ...
    pulse_divider = (uint8_t) round(256.0 / (float)current_brightness);
}
...
void leds_set_state(LED_STATE new_state) {
    ...
    logd(LOG_TAG, "setting led to %d %d %d, pulse period = %d, pulse delay = %d", col.r, col.g, col.b, pulse_period, current_pulse_delay);
    FastLED.show();
}
